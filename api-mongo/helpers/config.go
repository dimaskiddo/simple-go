...
func ConfigLoadValues(cfg *viper.Viper) {
	if cfg != nil {
		...
		MongoConfig.Host = cfg.GetString("DB_HOST")

		// Database Port
		cfg.SetDefault("DB_PORT", "27017")
		MongoConfig.Port = cfg.GetString("DB_PORT")

		// Database User
		cfg.SetDefault("DB_USER", "user")
		MongoConfig.User = cfg.GetString("DB_USER")

		// Database Password
		cfg.SetDefault("DB_PASSWORD", "password")
		MongoConfig.Password = cfg.GetString("DB_PASSWORD")

		// Database Name
		cfg.SetDefault("DB_NAME", "dbs")
		MongoConfig.Name = cfg.GetString("DB_NAME")
	} else {
